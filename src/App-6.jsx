    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 380, textAlign: 'center', animation: 'fadeUp 0.5s ease both' }}>
        ...
        <p style={{ color: '#333', fontSize: 12, marginTop: 16 }}>
          You will be asked to sign in to Google once.
        </p>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [driveAuthed, setDriveAuthed] = useState(() => localStorage.getItem('fieldsnap_drive') === '1');
  ...
  useEffect(() => {
    initGoogleDrive()
      .then(async (hasToken) => {
        ...
        if (hasToken) {
          setDriveAuthed(true);
        } else if (localStorage.getItem('fieldsnap_drive') === '1') {
          // Token expired, try silent refresh
          const refreshed = await silentRefresh();
          if (refreshed) {
            setDriveAuthed(true);
          } else {
            // Could not refresh silently, need user to re-auth once
            localStorage.removeItem('fieldsnap_drive');
  ...
  async function handleAuthorize() {
    ...
    try {
      ...
      setDriveAuthed(true);
      localStorage.setItem('fieldsnap_drive', '1');
    } catch (e) {
  ...
  async function handleEditPost({ images, district, note, dateTime }) {
    ...
    // Delete old file and re-upload with same id
    const folder = await window.gapi.client.drive.files.list({
      q: "name='post_" + updated.id + ".json' and trashed=false",
      fields: 'files(id)',
    });
    if (folder.result.files.length > 0) {
      const token = window.gapi.client.getToken().access_token;
      await fetch(
        'https://www.googleapis.com/upload/drive/v3/files/' + folder.result.files[0].id + '?uploadType=media',
        {
          method: 'PATCH',
